...
| 1.2 QR 重参数化                    | thin QR decomposition                | 薄 QR 分解          |                |                                             |        |           
| 1.5 Logistic 回归和 Probit 回归    | logistic regression                  | Logistic 回归       |                |                                             |        |
| 1.5 Logistic 回归和 Probit 回归    | probit regression                    | Probit 回归         |                |                                             |        |
...
| 1.9 分层回归                       | pooling                              | 汇集                |                |                                             |        |
| 1.11 项目-响应理论模型             | item-response model                  | 项目-响应模型       |                |                                             |        |
| 1.11 项目-响应理论模型             | informative prior                    | 有信息的先验        |                |                                             |        |
| 1.12 可识别先验                    | improper flat prior                  | 非正规的均匀先验    |                |                                             |        |
| 1.13 分层模型的多元先验            | group-level coefficients             | 组级系数            |                |                                             |        |
| 1.13 分层模型的多元先验            | scale vector                         | 尺度向量            |                |                                             |        |
| 3 缺失数据和部分已知参数           | dummy values                         | 虚拟值              |                |                                             |        |
| 5 有限混合模型                     | modes                                | 模态                |                |                                             |也可翻为“峰值”，暂保留原译         |
| 6 测量误差和元分析                 | meta analysis                        | 元分析              |                |                                             |        |
| 6.1 贝叶斯测量误差模型             | noninformative prior                 | 无信息的先验        |                |                                             |        |
| 9 聚类模型                         | cluster                              | 聚类                |                |                                             | 作为名词使用时，不翻译为“簇”       |
...
| 14 计算一维积分                    | integrator                           | 积分器              |                |                                             |        |
| 14 计算一维积分                    | evaluate                             | 求解                |                | 求方程、公式、函数的数值                    |        |
| 14 计算一维积分                    | norm                                 | 范数                |                |                                             |        |
| 14 计算一维积分                    | machine epsilon                      | 机械极小值          |                | 舍入误差                                    |        |
| 14 计算一维积分                    | quadrature                           | 求积                | 正交；求积；弦 |                                             |        |
| 14 计算一维积分                    | numerator                            | 式子（意译）        | 分子           | 上下文为定积分式子                          |        |
| 14 计算一维积分                    | the normalizing constant             | 标准化常量          | 归一化常数     | 先验概率                                    |        |
| 14 计算一维积分                    | argument                             | 实参/实际参数       |                | 调用的参数                                  |        |
| 14 计算一维积分                    | parameters                           | 形参/形式参数       |                | 函数中的参数                                |        |
| 14 计算一维积分                    | function signature                   | 函数签名            |                | 函数的参数和返回类型说明                    |        |
| 14 计算一维积分                    | call                                 | 调用                |                | 调用子程序                                  |        |
| 14 计算一维积分                    | Integration                          | 积分                |                | 泛指                                        |        |
| 14 计算一维积分                    | integral                             | 积分                |                | 特指                                        |        |
...
| 23 比例常数                        | Proportionality Constants            | 比例常数            |                |                                             |        |
