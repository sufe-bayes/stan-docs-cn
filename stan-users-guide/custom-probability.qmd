...
一个简单的例子是对称三角分布，其密度函数的形状像一个等腰三角形，三角形的底边由指定的边界确定，高度由密度函数积分为1的约束条件决定。$\alpha \in \mathbb{R}$ 和 $\beta \in \mathbb{R}$ 是边界，其中 $\alpha < \beta$，那么 $y \in (\alpha,\beta)$ 的密度函数定义为
...
由于此处分布的边界被指定为数据，因此 `-2 * log(beta - alpha)` 这一项可以从对数密度中移除。如果边界中的任意一个依赖于额外的参数，那么这一项就必须保留。
...
如果 $\alpha = -1$，$\beta = 1$，且 $y \in (-1,1)$，那么
`fmin(y - alpha, beta - y)` 即为 `fmin(y + 1, -1 - y)`，也就是 `fmin(y + 1, -(y + 1))`，它可以进一步化简为 `1 - abs(y)`。因此，在忽略了常数倍数后，密度函数可以表达为
