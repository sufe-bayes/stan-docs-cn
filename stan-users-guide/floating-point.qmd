...
由于存在符号位，零有两种表示方式，分别为“正零”和“负零”。这种区别在 Stan 中是无关紧要的（就像在 R 中一样），因为这两个值是相等的（即 `0 == -0` 求值为 true）。
...
Stan 提供了一个值函数 `not_a_number()`，它返回这个特殊的非数字值。它用于表示错误条件而不是缺失值。通常，当非数字是一个函数的参数时，如果没有抛出异常（在 Stan 中是拒绝），则结果将是非数字。
...
Stan 的大多数内置函数在它们的任何一个参数是非数字时将抛出异常（即拒绝）。
...
未定义的操作通常返回非数字值。例如，`sqrt(-1)` 会返回非数字。
...
正无穷和负无穷具有符合预期的比较行为，因此对 `negative_infinity() < 0` 求值的结果为 true（在 Stan 中用1表示）。此外，对正无穷取反会得到负无穷，反之亦然。
...
下溢是似然计算中普遍存在的问题。例如，如果 $p(y_n \mid \theta) < 0.1$，那么似然函数
