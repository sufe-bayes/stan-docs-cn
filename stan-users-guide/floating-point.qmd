...
在 $\mathbb{R}$ 语言中，计算机会使用固定的位数来近似实数值。本章将解释这是如何实现的，以及为什么编写稳健的 Stan（和其他数值）程序会非常重要。研究计算机上实数算术工作原理的计算机科学子领域被称为*数值分析*。
...
一个*正规*的浮点数值在尾数中不会使用零作为第一位；而*次正规*数第一位可能会使用零。并非所有的 I/O 系统都支持次正规数。
...
由于符号位的存在，有两种表示零的方式，通常被称为“正零”和“负零”。这种区别在 Stan 中是无关紧要的（就像在 R 中一样），因为这两个值是相等的(即 `0 == -0` 被认为是正确的)。
...
与非数字的比较会始终返回 false，包括与自身的比较。也就是说，`not_a_number() == not_a_number()` 会返回 false，尽管这会有些令人困惑。这就是为什么在 Stan（和C++）中需要一个内置的 `is_nan()` 函数。唯一的例外是不等于运算，它仍然连贯。这意味着 `not_a_number() != not_a_number()` 返回 true。
...
还有两个特殊值用于表示正无穷（$\infty$）和负无穷（$-\infty$）。它们不像非数字那样病态，但通常用于表示上溢和下溢等错误条件。例如，`log(0)` 不会引发错误或返回非数字，而是求值为负无穷。对负无穷进行指数化将返回零，因此会有 `0 == exp(log(0))`。然而，在 Stan 中不应该这样做，因为用于计算导数的链式法则将尝试非法操作并返回非数字。
...
在像 Stan 这样的编程语言中，数字可以用标准的十进制（基数为10）表示法来表示。例如，`2.39` 或 `-1567846.276452`。请记住，编写超过16个有效数字的做法是没有意义的，因为它们无法被表示。可以使用*科学计数法*来编码 Stan 中的数字，它由带符号的基数的十进制表示和带符号的整数十进制指数组成。例如，`36.29e-3`表示的数字是 $36.29 \times
...
对于接近1时数字四舍五入的问题，可以通过一些特殊的操作来减轻。例如，考虑对正数 `x` 进行操作 `log(1 + x)`。当 `x` 很小时（小于双精度浮点数的 $10^{-16}$），参数中的和将会四舍五入为1，结果将会四舍五入为零。为了提供更多的精度，编程语言提供了一个库函数来直接实现 $f(x) = \log (1 + x)$。在 Stan 中（与 C++ 相同），这个操作被写作 `log1p(x)`。由于 `x` 本身可能接近于零，函数 `log1p(x)` 可以取接近于1的值的对数，其结果接近于零。
...
在统计计算中，若一个分布具有相对于其位置而言很小的标准差，当我们计算该分布的方差时，会出现抵消灾难。在计算统计量时，Stan 使用 Welford 算法来计算方差。这避免了抵消灾难，并且可以在单次遍历中完成。
...
这就是为什么所有 Stan 中的概率函数都在对数尺度上进行操作。
...
尽管内部项本身可以使用内置函数 `log1p` 进行求值，但是这只会带来有限的收益，因为当 $u$ 远远大于 $v$时，$\exp(v - u)$ 会接近零，这意味着最终的结果很可能会四舍五入为 $u$。

To conclude, when evaluating $\log (a + b)$ given $\log a$ and $\log
b$, and assuming $\log a > \log b$, return

综上所述。当给定 $\log a$ 和 $\log b$ 来求解 $\log (a + b)$ 时，假 设$\log a > \log b$，会返回
$$
\log (a + b) =
...
Log-sum-exp 函数能够以明显的方式推广到序列，即如果 $v = v_1, \ldots, v_N$，那么
\begin{eqnarray*}
\textrm{log-sum-exp}(v)
