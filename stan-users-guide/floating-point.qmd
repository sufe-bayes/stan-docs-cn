...
# 浮点数算术 {#floating-point.chapter--cn}
...
*正规*浮点数的尾数部分不会以零开头；而*次正规*数第一位可能会使用零。并非所有的 输入/输出（I/O）系统都支持次正规数。
...
指数的某些取值被保留，因此合法指数的范围是 $-(2^{10}) + 2 = -1022$ 和 $2^{10} - 1 = 1023$ 之间。合法的尾数值介于 $-2^{52}$ 和 $2^{52} - 1$ 之间。浮点数允许表示非常大和非常小的值。一些极端值包括：
