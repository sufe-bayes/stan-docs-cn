...
一个表现良好的 Stan 程序在不同随机初始化和不同种子的随机数生成器运行之间具有较低的方差。但有时算法可能会卡在后验分布的一部分，通常是由于高曲率。这种停滞几乎总是表明需要重新对模型进行参数化。仅仅丢弃具有明显不良行为（缓慢或停滞）的马尔可夫链可能导致后验估计的偏差。这种陷入停滞的问题通常可以通过降低初始步长来克服，以避免在自适应过程中陷入困境，并增加目标接受率以达到较低的步长。这是因为较小的步长允许 Stan 的基于梯度的算法更好地遵循密度中的曲率或被惩罚的最大似然。
...
统计效率通常通过使模型的统计公式表现得更好来减少算法所需的步骤。实现这一点的典型方法是应用变量变换（即重新参数化），以便采样算法更好地混合，或者优化算法需要更少的自适应。
...
理想情况下，所有参数都应该进行编程，以具有单位尺度并减少后验相关性；总之，这些性质意味着 Stan 算法的最佳性能不需要旋转或缩放。对于哈密顿蒙特卡罗，这意味着一个单位质量矩阵，它不需要自适应，因为它是算法初始化的地方。
...
在除非常简单的模型（如多元正态）外的所有模型中，Hessian 将随着 $\theta$ 的变化而变化（一个极端的例子是 Neal 漏斗，这在数据很少或没有数据的分层模型中自然出现）。曲率变化越大，具有固定自适应参数的算法就越难找到覆盖整个密度的好自适应。所提出的许多变量变换旨在改善 Hessian 的条件，和/或使其在所拟合的密度（或惩罚最大似然函数）的相关部分之间更加一致。
...
统计效率的提高可以通过对模型进行重新参数化来实现，从而可以使用条件更好的密度或惩罚最大似然来计算相同的结果。再次，请参阅重新参数化 Neal 漏斗的示例，以及[变量变换章节](#change-of-variables.chapter)中的示例。
...
在使用最大似然估计时，必须小心使用变量变换重新参数化，因为如果雅可比行列式项无意中包含在修正的似然模型中，可能会改变结果。
...
Stan 的采样器在从具有困难后验几何的分布中采样时可能很慢。加快此类模型的一种方法是通过重新参数化。在某些情况下，重新参数化可以在相同的迭代次数下大大增加有效样本量，甚至可以使原本不会收敛的程序表现良好。

### Example: Neal's funnel {- #funnel.figure}

### 例：Neal 漏斗 {-#funnel.figure--cn}
...
在数据不多的情况下，这种重新参数化很有帮助，因为它将先验中的高层参数和低层参数分开。
...
Neal 的漏斗分布。（右图）4000次抽样取自 Stan 采样器的默认设置。两幅图都限制在所显示的 $x_1$ 和 $y$ 值的窗口内；有些抽样在所显示的区域之外，这是考虑到密度而预期的。这些样本与边际密度 $p(y)=\textsf{normal}(y \mid 0,3)$ 一致，其平均值为0，标准差为3。
:::
...
在这里的第二个模型中，参数 `x_raw` 和 `y_raw` 是按照独立标准正态采样的，这对 Stan 来说很容易。然后这些被转换为漏斗的采样样本。在这种情况下，同样的转换可以用来直接定义基于独立标准正态采样样本的蒙特卡洛样本；马尔科夫链蒙特卡洛方法不是必要的。如果在 Stan 代码中使用这样的重新参数化，那么提供一个说明参数分布对于转化后参数的分布的意义的注释是很有用的。
...
对于标准尺度上的许多应用，将使用位置参数为0、尺度参数为1的正态分布。在这些情况下，使用第一个比第二个代码更高效
