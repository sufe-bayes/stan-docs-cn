...
Stan 提供了多种不同的方法来求解常微分方程组（ODEs）。这些方法都可以自适应地优化其解以满足给定的容差，但在内部计算时它们的处理方式有所不同。
...
因为 Stan 中的算法需要对对数密度的梯度进行求解，ODE 求解器不仅要提供 ODE 本身的解，还要提供关于参数（灵敏度）的 ODE 解的梯度。在 Stan 中，有两种根本上不同的方法可用于解决此问题，具体的计算成本取决于正在使用的 ODE 状态数量 $N$ 和参数的数量 $M$ 等因素：
...
* 前向灵敏度求解器通过为解的梯度添加额外的 ODE 方程来扩展基本 ODE 系统。对于每个参数，都会添加一个完整的 $N$ 个灵敏度状态，这意味着所解的完整 ODE 具有 $N \, + N \cdot M$ 个状态。
...
* 伴随灵敏度求解器首先对基础 ODE 方程组进行正向求解，然后再对另一个 ODE 方程组（伴随方程）进行反向求解来获得梯度。正向和反向求解都各自拥有 $N$ 个状态。此外，每个参数都有一个数值积分问题需要求解。
...
伴随灵敏度方法比正向灵敏度方法的可扩展性要好得多。虽然正向方法的计算成本随着 ODE 状态数 $N$ 和参数数 $M$ 的乘法增长，而伴随灵敏度方法的计算成本随着状态数 $N$ 和参数数 $M$ 线性增长。然而，伴随问题更难配置，对于小问题来说其开销使得伴随灵敏度算法比解决完整的正向灵敏度方程组更慢。考虑到这一点，本文余下的部分将重点介绍正向灵敏度接口。有关伴随灵敏度接口的信息，请参阅[伴随ODE求解器](#adjoint-ode.section)。
...
每个正向灵敏度求解器都提供两个接口：一个带有默认容差和默认最大步数，一个允许修改这些控件。选择容差对于使任何求解器正常工作都非常重要-默认值不适用于所有情况。容差应首先考虑解的尺度、所需解的精度以及模型中使用解的方式。例如，如果解的分量在3.0和5.0之间缓慢变化，并且 ODE 状态的测量具有噪声，则容差可能不需要像解在3.0和3.1之间变化且可用高精度测量的情况下那么严格。当预计解的某个分量趋近于零时，减小绝对容差通常也很有用。有关选择容差的信息，请参阅[控制参数部分](#control-ode.section)。
...
有关刚性 ODE 系统的讨论，请参见[刚性ODE部分](#stiff-ode.section)。有关伴随灵敏度接口的信息，请参见[伴随ODE求解器部分](#adjoint-ode.section)。Stan的ODE求解器的函数签名可以在函数参考手册中的 ODE 求解器部分找到。
...
作为一个 ODE 系统的示例，先考虑一个谐波振荡器。在谐波振荡器中，被扰乱平衡的粒子被一个与平衡位移成比例的力拉回其平衡位置。这里的系统还有一个与粒子速度成正比的摩擦力，它指向粒子速度的相反方向。系统的状态由表示位置和速度的 $y = (y_1, y_2)$ 决定。系统相对于时间的变化由以下微分方程给出。
...
在 Stan 中编写 ODE 系统的第一步是定义 ODE 系统函数。系统函数需要特定的签名，以便求解器知道如何正确使用它们。
...
系统函数的第一个参数是时间，作为 `real` 传递；系统函数的第二个参数是系统状态，作为 `vector` 传递，并且系统函数的返回值是当前状态的时间导数，定义为 `vector`。可以在系统函数中包含其他参数以将其他信息传递到解决方案中（这些将通过启动 ODE 积分的函数传递）。这些参数可以是参数（在这种情况下是摩擦系数）、数据或任何需要定义微分方程的量。
...
ODE 系统函数中的类型是严格的。第一个参数是作为实数传递的时间，第二个参数是作为向量传递的状态，返回类型是向量。没有这个签名的模型将无法编译。从第三个参数开始，可以是任何类型，只要所有参数类型与求解器调用中相应参数的类型匹配即可。
...
系统参数 `theta` 和初始状态 `y0` 与初始时间 `t0` 和观测时间 `ts` 一起作为数据读入。ODE 在指定的时间内求解，然后添加随机测量误差以产生模拟观测值 `y_sim`。由于系统非刚性，因此使用 `ode_rk45` 求解器。
...
由于所有输入参数均不是参数的函数，因此在生成的数量块中调用 ODE 求解器。使用 `normal_rng` 将随机测量噪声添加到每个 `T` 输出中。
...
由于解现在是模型参数的函数，因此 `ode_rk45` 调用现在在模型块中进行。测量误差尺度 `sigma` 具有半正态先验，并且参数 `theta` 和初始状态向量 `y0` 具有标准正态先验。ODE 的解分配给 `mu`，作为正态观测模型的位置。
...
与其他回归模型类似，可以轻松将噪声模型更改为具有更重尾部的分布（例如，Student-t 分布），状态变量具有相关性的分布（例如，使用多元正态分布），或者状态变量具有重尾部和相关性的分布（例如，使用多元 Student-t 分布）。
...
刚性是一种数值现象，会导致某些微分方程求解器出现困难，尤其是先前示例中使用的 Runge-Kutta
RK45 求解器。这种现象在化学反应系统中很常见，这些系统通常具有多个截然不同的时间尺度。系统的刚性也可能因参数空间的不同部分而有所不同，因此通常非刚性的系统可能偶尔会表现出刚性。这些问题在宽松的先验或预热期间可能更频繁地发生。
...
Stan 为刚性 ODE 提供了专门的求解器 [@CohenHindmarsh:1996;
@SerbanHindmarsh:2005]。ODE 系统的指定方式完全相同，使用完全相同签名的函数。唯一的区别在于，在调用求解器时，`rk45` 后缀被替换为 `bdf`，例如：
...
如果不确定 ODE 系统是否具有刚性，请使用 `rk45` 和 `bdf` 求解器运行模型，并查看哪个更快。如果 `rk45` 求解器更快，那么这个问题可能是非刚性的，然后试试 `adams` 求解器也是有意义的。`adams` 求解器使用更高阶方法，可以比 `rk45` 求解器采用更大的时间步长，尽管与 `bdf` 求解器类似，这些步骤的计算更耗费时间
...
为了对求解器进行更多的控制，刚性和非刚性向前 ODE 求解器具有函数签名，使得可以指定 `relative_tolerance`、`absolute_tolerance` 和 `max_num_steps` 参数。这些与常规函数名称相同，但末尾添加了 `_tol`。所有三个控制参数必须使用此签名提供（没有默认值）。
...
如果 ODE 系统函数存在间断点，则最好在间断点之间积分 ODE，停止求解器并在另一侧重新启动。
...
尽管如此，ODE 求解器将尝试在状态函数中遇到的间断处进行积分。在间断点附近的解的精度可能存在问题（需要许多小步骤）。这种间断的一个例子是药代动力学模型中的延迟，在这个例子中，浓度值在时间点 $0<t<t'$ 为零，在 $t'\geq t$ 时为正。在这个例子中，我们可以在系统中使用如下代码：
...
最大迭代步数可用于停止失控的模拟。这可能发生在 MCMC 移动到参数空间中离通常求解微分方程的区域非常远的情况下。特别是在预热期间可能会发生这种情况。对于非刚性求解器，当采样器移动到参数空间的刚性区域时（需要较小的步长），这种情况可能会发生。
...
数学上讲，伴随 ODE 求解器方法与正向 ODE 求解器在获取 ODE 解的梯度方面存在差异。正向 ODE 方法对需要梯度的每个参数增加了 $N$ 个附加状态。如果需要 $M$ 个参数的灵敏度，则增加的 ODE 系统总共具有
$N \cdot (M + 1)$
...
个参数的情况，在后向求解期间还有 $M$
个额外的方程需要积分。由于这个原因，伴随灵敏度问题在参数数量上比正向灵敏度问题有更好的扩展性。Stan 中的伴随求解器使用 CVODES（与 `bdf` 和 `adams` 正向灵敏度接口相同）。
...
在前向积分中计算的解必须在后向积分中使用。CVODES 使用检查点方案定期保存前向求解器状态。在接口中，可以配置保存检查点之间的步数。然后，这些检查点使用两种插值方案之一在后向求解期间进行插值。
...
线性 ODE 系统可以使用矩阵指数解决。这比使用其中一个 ODE 求解器要快得多。
