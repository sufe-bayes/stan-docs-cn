...
有关刚性 ODE 系统的讨论，请参见[刚性 ODE 部分](#stiff-ode.section)。有关伴随灵敏度接口的信息，请参见[伴随 ODE 求解器部分](#adjoint-ode.section)。Stan的ODE求解器的函数签名可以在函数参考手册中的 ODE 求解器部分找到。
...
作为示例，假设有一个带有参数值 $\theta = 0.15$ 以及初始状态 $y(t = 0, \theta = 0.15) = (1, 0)$ 的简谐振子。假设该系统在10个时间点进行了测量，$t = 1, 2, \cdots, 10$，其中每个 $y(t, \theta)$ 的观测值在两个分量上 ($y_1(t, \theta)$
和$y_2(t, \theta)$) 都具有独立的 $\textsf{normal}(0, 0.1)$ 噪声。
...
为了对求解器进行更多的控制，刚性和非刚性向前 ODE 求解器具有函数签名，允许用户指定 `relative_tolerance`、`absolute_tolerance` 和 `max_num_steps` 参数。这些与常规函数名称相同，但末尾添加了 `_tol`。使用该签名时，三个控制参数都必须显式给出（不提供默认值）。
...
这些控制参数必须是数据变量 --
它们不能是参数或取决于参数的表达式，包括转换数据（transformed data ）和生成数量（generated quantities ）以外的任何块中的局部变量。用户定义的函数参数可以使用
`data` 限定符限定为仅允许数据参数。
