...
Stan 在支持实数标量、矩阵和向量的同时，也支持复数标量、矩阵和向量。
...
本节介绍复数标量类型的使用方法，包括如何构造复数、赋值复数，以及在数组和函数中的应用。
...
生成复数 $-1.1 + 2.3i$。 这种方式仅在实部和虚部都是字面量时有效。要从任意实变量中构造一个复数，可以使用 `to_complex()` 函数。例如，如果 `x` 和 `y` 是函数或模型块中的参数、转换数据或局部变量，则以下代码将起作用。
...
类型提升同样适用于复数函数参数，可以传入整数或实数值，这些值会在函数执行前自动提升为复数。例如，下面这个用户定义的复数函数可以接受整数、实数或复数参数。
...
对复数随机变量 $z = x + yi$ 的分布进行建模的最简单方法是将其实部 $x$ 和虚部 $y$ 视为具有二元正态分布。 例如，一个复数的先验可以表示如下。
...
Stan 支持复数矩阵、向量和行向量。这些类型的变量大小的定义方式与实数类型相同。
...
复矩阵和向量支持所有标准的算术运算，包括取负、加法、减法和乘法（除法涉及方程的求解，而不是矩阵之间简单的算术运算）。 它们还支持转置。
