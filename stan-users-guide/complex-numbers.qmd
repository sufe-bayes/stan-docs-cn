...
生成复数 $-1.1 + 2.3i$。 这仅在实部和虚部是字面量时才有效。 要从任意实变量中构造一个复数，可以使用 `to_complex()` 函数。 例如，如 果`x` 和 `y` 是函数或模型块中的参数、转换数据或局部变量，则以下代码将起作用。
...
在这里，误差的实部和虚部是具有相关和独立尺度的联合分布。该误差服从均值为零的多元正态分布和一个 Cholesky 因子协方差表示，其由尺度向量 `sigma` 和 Cholesky 因子或相关矩阵 `L_Omega` 构成。相关性的先验松散地分布在对角协方差附近，尺度上的先验未被指定。为了对 `multi_normal_cholesky` 的调用进行矢量化操作，需要将复数向量转换为大小为2的向量数组。
